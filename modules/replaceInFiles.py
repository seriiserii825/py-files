...
def replaceInFiles():
    ...
    if len(attached_dirs) > 0:
        ...
        command = f"find {' '.join(attached_dirs)} -type f -name '*{file_extension}' -not -path \"{excluded_dirs}\" -exec grep -l '{search_string}' {{}} \\; | xargs sed -i 's/{search_string}/{replace_string}/g'"
        ...
        command = f"find . -type f -name '*{file_extension}' -not -path \"{excluded_dirs}\" -exec grep -l '{search_string}' {{}} \\; | xargs sed -i 's/{search_string}/{replace_string}/g'"
        print(Panel(f"command: {command}"))
        agree = console.input("[blue]Do you want to run the command, (y/n): ")
        if agree.lower() == 'y':
            os.system(command)
